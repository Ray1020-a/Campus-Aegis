...
const systemAndUser = (flowJson) => `你是一個網路安全分析師。你會收到一段時間內的「流量彙總」列表，每筆包含：src（來源IP）、dst（目的IP）、sport/dport（port）、protocol、packets、bytes。
...
function isQuotaError(err) {
  ...
  return code === 429 || msg.includes('429') || msg.includes('RESOURCE_EXHAUSTED') || msg.includes('quota') || msg.includes('rate');
}

function parseRetryDelaySeconds(err) {
  try {
    const msg = err?.message ?? String(err);
    const m = msg.match(/retry in (\d+(?:\.\d+)?)\s*s/i) || msg.match(/"retryDelay":"(\d+)s"/);
    ...
    if (Array.isArray(details)) {
      const retryInfo = details.find((d) => d && (d['@type']?.includes('RetryInfo') || d.retryDelay));
      if (retryInfo?.retryDelay) {
        const sec = Number(String(retryInfo.retryDelay).replace(/s$/, '')) || DEFAULT_RETRY_SEC;
...
export async function analyzeWithGemini(flowSummary) {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      const raw = resp?.text ?? '';
      const trimmed = raw.replace(/^[\s`]*|[\s`]*$/g, '').replace(/^.*?(\[.*\]).*$/s, '$1');
      try {
        const arr = JSON.parse(trimmed);
        if (!Array.isArray(arr)) return [];
        return arr.filter((x) => typeof x === 'string' && /^[\d.]+$/.test(x)).map((x) => `${x}/32`);
      } catch (_) {
        return [];
      }
    } catch (err) {
      ...
      if (attempt < MAX_RETRIES && isQuotaError(err)) {
        ...
        console.warn(`[Gemini] 配額/頻率限制 (429)，${sec} 秒後重試 (${attempt + 1}/${MAX_RETRIES})…`);
